...
if __name__ == '__main__':
    dataset_path = 'data/alipay/user_log_sorted.csv'

    if not os.path.exists('data/alipay/uid_mapping.npy') or \
            not os.path.exists('data/alipay/iid_mapping.npy') or \
            not os.path.exists('data/alipay/cid_mapping.npy'):
        uid_mapping, iid_mapping, cid_mapping = {}, {}, {}

        with open(dataset_path, 'r') as file:
            for line in file:
                uid, _, iid, cid, btype, ts = line.split(',')
                if btype == 'act_ID' or int(btype) != 0:
                    continue
        ...
        np.save('data/alipay/uid_mapping.npy', uid_mapping)
        np.save('data/alipay/iid_mapping.npy', iid_mapping)
        np.save('data/alipay/cid_mapping.npy', cid_mapping)
    else:
        uid_mapping = np.load('data/alipay/uid_mapping.npy', allow_pickle=True).item()
        iid_mapping = np.load('data/alipay/iid_mapping.npy', allow_pickle=True).item()
        cid_mapping = np.load('data/alipay/cid_mapping.npy', allow_pickle=True).item()
    ...
    for uid in tqdm(user_iid_hist):
        ...
        # if len(user_iid_hist[uid]) > 50:
        #     valid_user_num += 1
        # print(valid_user_num)
        if neg_idx > 99000:
            all_rnd_neg = random.choices(population, p, k=100000)
            neg_idx = 0
    ...
    print('n_user:', len(uid_mapping))  # 626041
    print('n_item:', len(iid_mapping))  # 2200291
    print('n_multi_cid_item:', len(multi_cid_item))  # 0
    print('n_cate:', len(cid_mapping))  # 72
    print('n_train:', len(train_npy))  # 3226656
    print('n_val:', len(val_npy))  # 111264
    print('n_test:', len(test_npy))  # 111264
    ...
    for i in range(0, len(train_npy), 102400):
        ...
        while len(name) < 4:
            name = "0" + name
        np.save('data/alipay/train_210_30_'+name+'_'+str(len(train_npy[i:i+102400]))+'.npy', np.array(train_npy[i:i + 102400], dtype=object))
        train_data_index += 1

    np.save('data/alipay/val_210.npy', np.array(val_npy, dtype=object))
    np.save('data/alipay/test_210.npy', np.array(test_npy, dtype=object))
