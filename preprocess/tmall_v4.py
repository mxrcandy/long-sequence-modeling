...
if __name__ == '__main__':
    ...
    if not os.path.exists('data/tmall/uid_mapping.npy') or \
            not os.path.exists('data/tmall/iid_mapping.npy') or \
            not os.path.exists('data/tmall/cid_mapping.npy'):
        ...
        np.save('data/tmall/uid_mapping.npy', uid_mapping)
        np.save('data/tmall/iid_mapping.npy', iid_mapping)
        np.save('data/tmall/cid_mapping.npy', cid_mapping)
    else:
        uid_mapping = np.load('data/tmall/uid_mapping.npy', allow_pickle=True).item()
        iid_mapping = np.load('data/tmall/iid_mapping.npy', allow_pickle=True).item()
        cid_mapping = np.load('data/tmall/cid_mapping.npy', allow_pickle=True).item()
    ...
    print('n_user:', len(uid_mapping))  # 423862
    print('n_item:', len(iid_mapping))  # 1080666
    print('n_multi_cid_item:', len(multi_cid_item))  # 3919
    print('n_cate:', len(cid_mapping))  # 1492
    print('n_train:', len(train_npy))  # 3300870
    print('n_val:', len(val_npy))  # 173730
    print('n_test:', len(test_npy))  # 173730
    ...
    for i in range(0, len(train_npy), 102400):
        ...
        while len(name) < 4:
            name = "0" + name
        np.save('data/tmall/train_210_' + name + '_' + str(
            len(train_npy[i:i + 102400])) + '.npy', np.array(train_npy[i:i + 102400], dtype=object))
        train_data_index += 1


    np.save('data/tmall/val_210.npy', np.array(val_npy, dtype=object))
    np.save('data/tmall/test_210.npy', np.array(test_npy, dtype=object))
